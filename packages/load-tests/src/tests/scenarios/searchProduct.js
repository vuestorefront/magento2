...
export default function main() {
  ...
  group(`Search Product - page ${BASE_URL}`, () => {
    group("Visit the homepage", () => {
      res = http.get(`${BASE_URL}/default`, { headers });
      check(res, {
        "is status 200": (r) => r.status === 200,
      });
    ...
    group("Search for a product", () => {
      res = http.post(`${BASE_URL}/api/magento/products`, '[{"pageSize":12,"search":"erika"},{"products":"products"}]', { headers });
      executeCommonChecks(res);
      sleep(1.9);
    });
    ...
    group("Visit the found product", () => {
      ...
      res = http.post(`${BASE_URL}/api/magento/relatedProduct`, '[{"filter":{"sku":{"eq":"WSH12"}}},null]', { headers });
      executeCommonChecks(res);

      res = http.post(`${BASE_URL}/api/magento/upsellProducts`, '[{"filter":{"sku":{"eq":"WSH12"}}},null]', { headers });
      executeCommonChecks(res);

      res = http.post(`${BASE_URL}/api/magento/productReviewRatingsMetadata`, "[null]", { headers });
      executeCommonChecks(res);
    });
  });
}
