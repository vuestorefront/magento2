...
export default function main() {
  ...
  group(`Browsing for a product - page ${BASE_URL}`, () => {
    ...
    group("Go to the Men category page", () => {
      res = http.post(`${BASE_URL}/api/magento/route`, '["/men.html",null]', {
        headers,
      });
      check(res, {
        "is status 200": (r) => r.status === 200,
    ...
    group("Visit selected product", () => {
      ...
      res = http.post(`${BASE_URL}/api/magento/upsellProducts`, '[{"filter":{"sku":{"eq":"MH06"}}},null]', {
        headers,
      });

      res = http.post(`${BASE_URL}/api/magento/relatedProduct`, '[{"filter":{"sku":{"eq":"MH06"}}},null]', {
        headers,
      });
      executeCommonChecks(res);
