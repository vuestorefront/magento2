...
import { CustomerUpdateInput } from '@vsf-enterprise/magento-api-types';
...
describe(describeGroup('updateCustomer'), () => {
  ...
  it('should update customer data', async () => {
    ...
    const { data } = await sdk.magento.updateCustomer(newCustomerData, { customHeaders: { Authorization: `Bearer ${token}` } });
  ...
  it('should update customer data using custom query', async () => {
    ...
    const { data } = await sdk.magento.updateCustomer(newCustomerData, { customQuery, customHeaders: { Authorization: `Bearer ${token}` } });
