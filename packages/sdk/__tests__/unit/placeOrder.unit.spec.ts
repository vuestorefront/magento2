...
import { PlaceOrderInput } from '@vsf-enterprise/magento-api-types';

const PARAMS_MOCK: PlaceOrderInput = {
  cart_id: 'cart_id'
};
const OPTIONS_MOCK = { clientConfig: {}, customHeaders: {}, customQuery: {} } as MethodOptions<CustomQuery<'placeOrder'>>;
...
describe(describeGroup('placeOrder'), () => {
  ...
  it('makes a call to API Middleware with proper params and options', async () => {
    ...
    expect(client.post).toBeCalledWith(
      'placeOrder', [expect.objectContaining(PARAMS_MOCK), {}], {}
    );
  });
