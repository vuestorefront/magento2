import { getAvailableShippingMethods } from '../../src/methods';
...
describe(describeGroup('getAvailableShippingMethods'), () => {
  it('makes a single call to API Middleware', async () => {
    await getAvailableShippingMethods(PARAMS_MOCK);
  ...
  it('extracts and returns a response', async () => {
    const response = await getAvailableShippingMethods(PARAMS_MOCK, OPTIONS_MOCK);
