...
import { GetProductSearchParams } from '@vsf-enterprise/magento-api-types';
...
describe(describeGroup('productReview'), () => {
  it('makes a single call to API Middleware', async () => {
    await productReview(PARAMS_MOCK);
  ...
  it('extracts and returns a response', async () => {
    const response = await productReview(PARAMS_MOCK, OPTIONS_MOCK);
