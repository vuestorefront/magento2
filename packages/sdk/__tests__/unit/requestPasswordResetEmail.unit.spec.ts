import { requestPasswordResetEmail } from '../../src/methods';
...
describe(describeGroup('requestPasswordResetEmail'), () => {
  it('makes a single call to API Middleware', async () => {
    await requestPasswordResetEmail(PARAMS_MOCK);
  ...
  it('extracts and returns a response', async () => {
    const response = await requestPasswordResetEmail(PARAMS_MOCK, OPTIONS_MOCK);
